data {
  ...
  real<lower=0> sigma_orth; // Not used anymore
...
transformed data {
  ...
  for (b in 1:B)
    if (N_b[b] > max_N_b) max_N_b = N_b[b];
...
parameters {
  ...
  vector<lower=0>[B] tau_b;
  matrix[N, K] H_raw;
  matrix[P, K] Lambda;
  vector<lower=0>[K] kappa;
  matrix[B, P] z_log_delta;
  vector<lower=0>[B] sigma_logdelta_b;
  matrix[B, K] z_log_s;
...
transformed parameters {
  ...
  for (b in 1:B)
    log_delta_raw[b] = sigma_logdelta_b[b] * z_log_delta[b];
...
model {
  ...
  tau_b ~ gamma(a_gamma, b_gamma);

  for (b in 1:B)
    gamma_raw[b] ~ normal(0, tau_b[b]);
  ...
  sigma_logdelta_b ~ normal(0, sigma_logdelta_prior);
