...
transformed parameters {
  ...
  if (K > 0) {
    for (b in 1:B) {
      int n_count = N_b[b];
      H[idx_b[b, 1:n_count]] =
        H_raw[idx_b[b, 1:n_count]] .* rep_matrix(s[b], n_count);
    }
  }
}
...
model {
  ...
  if (K > 0) to_vector(H_raw) ~ std_normal();
  if (K > 0) {
    for (k in 1:K) {
  ...
  to_vector(log_s_raw) ~ normal(0, sigma_logs);
  ...
  if (K > 0 && sigma_orth > 0) {
    for (b in 1:B) {
      int n_count = N_b[b];
      for (k in 1:K) {
        real h_mean = mean(col(H_raw[idx_b[b, 1:n_count]], k));
...

  {
    ...
    for (b in 1:B) {
      ...
      if (K > 0) Mub += H[idx_b[b, 1:n_count]] * Lambda';
...
generated quantities {
  ...
  for (g in 1:P) {
    real tmp = 0;
    for (b in 1:B) tmp += w_b[b] * log(delta[b, g]);
    delta_bar[g] = exp(tmp);
  }
