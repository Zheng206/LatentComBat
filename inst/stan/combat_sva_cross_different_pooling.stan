...
parameters {
  ...
  // Regularized latent batch-scale effects.
  // We intentionally do not estimate a separate global SD hyperparameter
  // sigma_logs because repeated fits showed that this variance component
  // was weakly identified near zero and was the sole source of poor R-hat/ESS.
  // The data input sigma_logs_prior now directly controls the prior SD of
  // log-scale deviations.
  matrix[B, K] z_log_s;
...
transformed parameters {
  ...
  if (K > 0) {
    // Regularized latent batch scaling.
    // Equivalent to log_s_raw[b,k] ~ normal(0, sigma_logs_prior),
    // but parameterized through standard-normal z scores for stable sampling.
    log_s_raw = sigma_logs_prior * z_log_s;
    {
...
model {
  ...
  // Latent batch-scale regularization.
  // sigma_logs_prior is a fixed regularization scale supplied as data;
  // no additional weakly identified global variance component is estimated.
  if (K > 0)
    to_vector(z_log_s) ~ std_normal();

  // Residual batch scaling retains its estimable hierarchical SD because
  // this component has shown good identification in diagnostics.
